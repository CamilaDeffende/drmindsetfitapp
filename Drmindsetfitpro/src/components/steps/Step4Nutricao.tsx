...
import { ALIMENTOS_DATABASE, buscarAlimento, calcularMacros, buscarSubstituicoes } from '@/types/alimentos'
...
export function Step4Nutricao() {
  ...
  const gerarPlanejamento = () => {
    ...
    const caloriasRefeicao = Math.round(caloriasFinais / refeicoesSelecionadas.length)
    const proteinaRefeicao = Math.round(proteina / refeicoesSelecionadas.length)
    const carbosRefeicao = Math.round(carboidratos / refeicoesSelecionadas.length)
    const gordurasRefeicao = Math.round(gorduras / refeicoesSelecionadas.length)
    ...
    <div className="max-w-4xl mx-auto px-4 py-6 sm:py-8">
      ...
      <Card className="mb-4 sm:mb-6">
        ...
        <CardContent>
          ...
          <Select value={estrategia} onValueChange={(v: any) => setEstrategia(v)}>
