#!/usr/bin/env node
/**
 * BLOCO 6B FIX4 — aplicar UI completa + equivalências
 * Correção: sanity de hooks agora valida hooks SOMENTE dentro de helpers (não no arquivo inteiro).
 */
const fs = require("fs");
...
if (endIdx < 0) die("Não consegui fechar o bloco do componente.");
...
// achar return (qualquer forma) para manter “head”
const mReturn = comp.match(/\breturn\b/);
const returnPos = mReturn ? comp.indexOf(mReturn[0]) : -1;
const lastBracePos = comp.lastIndexOf("}");
if (lastBracePos < 0) die("Componente sem '}' final.");

const head = comp.slice(0, returnPos >= 0 ? returnPos : lastBracePos).trimEnd();
const tail = comp.slice(lastBracePos); // "}"
...
// injeta antes do '}' final
const newComp = head + CALC + "\n" + RETURN + "\n" + tail;
s = s.slice(0, startIdx) + newComp + s.slice(endIdx+1);

// sanity: marcador
if (!s.includes("MF_BLOCO6B_DIETA_UI")) die("Sanity: marcador MF_BLOCO6B_DIETA_UI não encontrado.");

// sanity CORRETA: verificar hooks apenas dentro do helper mfEqPortion (se existir)
...
write(FILE, s);
console.log("✅ FIX4 aplicado em:", FILE);
