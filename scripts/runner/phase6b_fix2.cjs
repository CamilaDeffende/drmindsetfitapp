#!/usr/bin/env node
/**
 * BLOCO 6B FIX2 — aplicar UI completa NO COMPONENTE (evitar rules-of-hooks)
 * Sem template literal gigante (evita SyntaxError do Node).
 */
const fs = require("fs");
...
let comp = s.slice(startIdx, endIdx+1);

if (!/const\s+navigate\s*=\s*useNavigate\(\)/.test(comp)) {
  comp = comp.replace(/\{\s*\n/, "{\n  const navigate = useNavigate();\n");
}

if (comp.search(/return\s*\(/) < 0) die("Não achei return( dentro do componente.");
...
const REPL = [
  ...
  "              <div className=\"text-sm text-white mt-1\">",
  "                {alvoKcal > 0 ? (",
  "                  <span>Diferença: <span className=\"font-semibold\">{mfRound(day.kcal - alvoKcal)} kcal</span></span>",
  "                ) : (",
...
// troca apenas o return do componente
comp = comp.replace(/return\s*\([\s\S]*?\);\s*$/m, REPL);

s = s.slice(0, startIdx) + comp + s.slice(endIdx+1);

if (!s.includes("MF_BLOCO6B_DIETA_UI")) die("Sanity: marcador MF_BLOCO6B_DIETA_UI não encontrado.");
if (/mfEqPortion[\s\S]*useMemo/.test(s)) die("Sanity: hooks caíram fora do componente (não pode).");

if (s === before) die("Nenhuma alteração aplicada (inesperado).");
write(FILE, s);
console.log("✅ FIX2 aplicado em:", FILE);
