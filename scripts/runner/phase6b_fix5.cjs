#!/usr/bin/env node
/**
 * BLOCO 6B FIX5 — aplicar UI completa + equivalências (sem quebrar assinatura)
 * Fix crítico: achar o '{' DO CORPO após fechar ')' da assinatura, não o '{' do destructuring.
 */
const fs = require("fs");
...
// localizar assinatura do componente
const startIdx = s.search(/export\s+function\s+DietaAtivaView\s*\(/);
if (startIdx < 0) die("Não achei: export function DietaAtivaView(");

// achar '(' da assinatura e fechar ')'
const parenOpen = s.indexOf("(", startIdx);
if (parenOpen < 0) die("Não achei '(' da assinatura.");
let p = parenOpen, depthP = 0, parenClose = -1;
for (; p < s.length; p++){
  const ch = s[p];
  if (ch === "(") depthP++;
  else if (ch === ")") {
    depthP--;
    if (depthP === 0) { parenClose = p; break; }
  }
}
if (parenClose < 0) die("Não consegui fechar ')' da assinatura.");

// agora sim: '{' do CORPO após o ')'
const bodyOpen = s.indexOf("{", parenClose);
if (bodyOpen < 0) die("Não achei '{' do corpo após ')'.");

// achar '}' final do corpo
let i = bodyOpen, depth = 0, bodyEnd = -1;
for (; i < s.length; i++){
  const ch = s[i];
  if (ch === "{") depth++;
  else if (ch === "}") {
...
if (bodyEnd < 0) die("Não consegui fechar o corpo do componente.");

let comp = s.slice(startIdx, bodyEnd+1);

// garantir navigate dentro do corpo
if (!/const\s+navigate\s*=\s*useNavigate\(\)\s*;?/.test(comp)){
  comp = comp.replace(/\{\s*\n/, "{\n  const navigate = useNavigate();\n");
}

// encontrar o RETURN de nível do componente (indent 2 espaços)
const mTopReturn = comp.match(/\n\s{2}return\b/);
if (!mTopReturn) die("Não achei o return do componente (nível topo).");

const returnPos = comp.indexOf(mTopReturn[0]) + 1; // inclui o '\n' pra manter formatação
const lastBrace = comp.lastIndexOf("}");
if (lastBrace < 0) die("Componente sem '}' final.");

const head = comp.slice(0, returnPos).trimEnd();
const tail = comp.slice(lastBrace); // "}"
...
const newComp = head + CALC + "\n" + RETURN + "\n" + tail;

// reconstroi no arquivo
s = s.slice(0, startIdx) + newComp + s.slice(bodyEnd+1);

// sanity: marcador
if (!s.includes("MF_BLOCO6B_DIETA_UI")) die("Sanity: marcador MF_BLOCO6B_DIETA_UI não encontrado.");

// sanity: não pode ter hook dentro do helper
const mEq = s.match(/function\s+mfEqPortion\s*\([^)]*\)\s*\{([\s\S]*?)\n\}/m);
if (mEq){
  const body = mEq[1] || "";
  if (/\buseMemo\b|\buseState\b|\buseDrMindSetfit\b/.test(body)) {
    die("Sanity: hooks indevidos dentro de mfEqPortion (não pode).");
  }
}

if (s === before) die("Nenhuma alteração aplicada (inesperado).");
write(FILE, s);
console.log("✅ FIX5 aplicado em:", FILE);
