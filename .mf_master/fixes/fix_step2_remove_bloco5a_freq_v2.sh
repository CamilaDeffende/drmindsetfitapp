...
# achar a primeira linha com "<div" depois do marcador (abre o quadro)
open_idx = None
for i in range(start, len(lines)):
    if "<div" in lines[i]:
        ...
        break
if open_idx is None:
    raise SystemExit("❌ Não achei nenhuma <div> após o marcador.")
...
if end is None:
    raise SystemExit("❌ Não consegui achar o fechamento do quadro (</div>).")

out = lines[:start] + ["\n"] + lines[end+1:]
s2 = "".join(out)

# sanity correto: só marcadores exclusivos do bloco
