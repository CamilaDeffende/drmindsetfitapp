...
cat > .scripts/block3_hotfix_revert_state_calls.mjs <<'MJS'
...
echo "==> Rodar hotfix patcher..."
node .scripts/block3_hotfix_revert_state_calls.mjs
...
echo "==> COMMIT + PUSH..."
git add -A
git commit -m "fix(weekplan): hotfix revert invalid state calls (restore build green)" || echo "ℹ️ Nada para commitar."
git push origin "$BR"
