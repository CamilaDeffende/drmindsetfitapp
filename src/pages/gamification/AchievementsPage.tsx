...
function mapLegacyCategory(achievement: LegacyAchievement): UiAchievement["category"] {
  if (achievement.id === "NUTRITION_PLAN_SET") return "nutricao";
  if (achievement.id === "AUDIT_ACTIVE") return "auditoria";
  if (achievement.id === "FIRST_WORKOUT" || achievement.id === "THREE_WORKOUTS_WEEK" || achievement.id === "SEVEN_DAY_STREAK") {
    return "treino";
  }
  return "geral";
}
...
function mergeAchievements(): UiAchievement[] {
  ...
  for (const item of all) {
    const key = canonicalKeyMap.get(item.id) ?? `${item.title}::${item.description}`;
  ...
  return Array.from(unique.values()).sort((a, b) => Number(b.unlocked) - Number(a.unlocked));
}

export function AchievementsPage() {
  const navigate = useNavigate();
  const [level, setLevel] = useState(levelSystem.getProgress());
  const [achievements, setAchievements] = useState<UiAchievement[]>(() => mergeAchievements());
        ...
        <Card className="mt-6 border-white/10 bg-[rgba(8,10,18,0.82)]">
          ...
          <CardContent>
            ...
            <div className="mt-3 flex flex-wrap gap-2 text-xs text-white/55">
              ...
              <span>•</span>
              <span>Treino, nutricao e auditoria no mesmo painel</span>
            </div>
          </CardContent>
        </Card>
