...
export function PlanosAtivos() {
  ...
  const __mfActivePlanRaw =
    typeof window !== "undefined" ? localStorage.getItem("mf:activePlan:v1") : null;
  ...
  const treinoPlan =
    (state as any)?.treino ?? (state as any)?.workoutProtocolWeekly?.treinoPlan ?? null;
  ...
  }, [state.nutricao, state.treino, state.dietaAtiva, state.treinoAtivo, updateState, adapted, state]);

  if (!state.concluido) {
    const __mfWeeklyPlan =
      typeof buildDeterministicWeeklyPlan === "function"
        ? buildDeterministicWeeklyPlan(state as any)
        : typeof mfBuildWeeklyPlanFromState === "function"
        ? mfBuildWeeklyPlanFromState(state as any)
        : null;

    const __mfSessions = Array.isArray((__mfWeeklyPlan as any)?.sessions)
      ? (__mfWeeklyPlan as any).sessions
      : [];

    const __mfModalities = Array.from(
      new Set(__mfSessions.map((x: any) => String(x?.modality ?? ""))).values()
    ).filter(Boolean);

    const __mfPlanLevel = __mfModalities.length <= 1 ? "foco" : "misto";

    const __mfHasMulti =
      Array.isArray((state as any)?.workoutModalities) &&
      (state as any).workoutModalities.length > 0;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-black">
        <section style={{ marginTop: 16 }}>
          ...
          >
            <h2 style={{ fontSize: 18, fontWeight: 800, letterSpacing: 0.2 }}>
              Atividades físicas ativas
            </h2>
            <span style={{ opacity: 0.75, fontSize: 12 }}>
              Modalidades • Dias • Intensidade
            </span>
          </div>

          {!__mfTrainingSummaries || __mfTrainingSummaries.length === 0 ? (
            <div style={{ marginTop: 10, opacity: 0.85 }}>
              Nenhuma modalidade ativa encontrada ainda. Conclua o onboarding para gerar seus
              treinos.
            </div>
          ) : (
            <div
            ...
            >
              {__mfTrainingSummaries.map((t) => (
                ...
                >
                  ...
                  {t.note ? (
                    <div style={{ marginTop: 10, fontSize: 13, opacity: 0.8 }}>{t.note}</div>
                  ) : null}
                </div>
              ))}
            </div>
                ...
                {__mfWorkouts.map((w, i) => (
                  ...
                  >
                    <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}>
                      ...
                      <div style={{ fontSize: 12, opacity: 0.8 }}>
                        {w.intensity || "Auto"} • {w.level || "Auto"}
                      </div>
                    </div>
                    <div style={{ marginTop: 6, fontSize: 13, opacity: 0.9 }}>
                      {w.title || "Treino do dia"}
                    </div>
                    {w.focus ? (
                      <div style={{ marginTop: 4, fontSize: 12, opacity: 0.75 }}>{w.focus}</div>
                    ) : null}
                    {w.durationMin ? (
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-3">
                ...
                <div className="rounded-xl bg-white/5 p-3">
                  ...
                  <div className="mt-1 text-sm text-white/80">
                    {macros
                      ? `${macros.protein ?? "—"}g P • ${macros.carbs ?? "—"}g C • ${
                          macros.fat ?? "—"
                        }g G`
                      : "—"}
                  </div>
                ...
                <div className="rounded-xl bg-white/5 p-3">
                  ...
                  <div className="mt-1 text-sm text-white/80">
                    {Array.isArray(meals) ? meals.length : 0} / dia
                  </div>
                </div>
              </div>
            ) : (
              <div className="mt-3 rounded-xl bg-white/5 p-3 text-sm text-white/70">
                Nenhum plano ativo encontrado ainda. Finalize o onboarding e confirme para salvar o
                plano.
              </div>
            )}
        ...
        <div className="mt-4 rounded-2xl border border-white/10 bg-white/5 p-4 sm:p-5 space-y-3">
            ...
            {!HIDE_ADVANCED_MODALITY_UI && (
              <div>
                ...
                <p className="text-xs sm:text-sm text-muted-foreground leading-relaxed">
                  Opcional. Combine dois focos na semana (ex.: musculação + crossfit). Se não
                  quiser, selecione “Sem modalidade secundária”.
                </p>
              </div>
            )}
          ...
          <p className="text-[11px] text-muted-foreground">
            Isso cria um segundo protocolo completo de treino (semanal), respeitando seu nível e os
            dias escolhidos.
          </p>
        </div>
        ...
        <div className="mt-4 rounded-2xl border border-white/10 bg-white/5 p-4 sm:p-5 space-y-3">
          <div className="flex items-start justify-between gap-3">
            {!HIDE_ADVANCED_MODALITY_UI && (
              <div>
                ...
                <p className="text-xs sm:text-sm text-muted-foreground leading-relaxed">
                  Selecione em quais dias você quer executar cada modalidade. Se você não marcar
                  nada, a distribuição automática continua valendo.
                </p>
              </div>
            )}
            <span className="text-[11px] px-2 py-1 rounded-full border border-white/10 bg-white/5 text-muted-foreground">
          ...
          {(() => {
            ...
            const schedule = ((state as any)?.workoutScheduleByModality ?? {}) as Record<
              string,
              string[]
            >;
            ...
            if (!selected.length) {
              return (
                <div className="text-sm text-muted-foreground">
                  Selecione ao menos uma modalidade para configurar os dias.
                </div>
              );
            }
            ...
            const toggle = (modKey: string, dayKey: string) => {
              ...
              const next = cur.includes(dayKey)
                ? cur.filter((d) => d !== dayKey)
                : [...cur, dayKey];
                    ...
                    <div className="text-[11px] text-muted-foreground flex items-center gap-2">
                      ...
                      {(() => {
                        try {
                          ...
                          const __label =
                            __k && typeof MODALITIES !== "undefined"
                              ? MODALITIES.find((m) => m.key === __k)?.label ?? __k
                              : __k;
                        ...
                        <div className="mt-3 space-y-2">
                          {(b.main || []).slice(0, 8).map((it: any, ii: number) => (
                            ...
                            >
                              ...
                              <div className="text-muted-foreground whitespace-nowrap">
                                {it.time
                                  ? it.time
                                  : String(it.sets ?? "") + "x " + String(it.reps ?? "")}{" "}
                                {it.rest ? "• " + it.rest : ""}
                              </div>
                            </div>
                          ))}
                          {Array.isArray(b.main) && b.main.length > 8 ? (
                            <div className="text-[11px] text-muted-foreground">
                              + {b.main.length - 8} exercícios (variações no plano)
                            </div>
                          ) : null}
                        </div>
                      </div>
                  ...
                  <TabsContent value="dieta" className="mt-0">
                    ...
                    <div className="mt-4">
                      ...
                      <p className="mt-2 text-xs text-gray-400">
                        Ajuste refeições, alimentos e equivalências mantendo a consistência calórica
                        do plano.
                      </p>
                    </div>
                  </TabsContent>
                    ...
                    <div data-testid="active-workout-protocol" className="mt-4 space-y-3">
                      {!treinoPlan ? (
                        <div className="rounded-xl border border-border/50 p-4 text-sm text-muted-foreground">
                          Nenhum treino ativo gerado ainda. Finalize o onboarding e clique em{" "}
                          <strong>Gerar Treino</strong> no Step 5.
                        </div>
                      ) : (
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
                                  <div>
                                    <p className="text-sm font-semibold">
                                      {dia?.dia} • {dia?.modalidade}
                                    </p>
                                    <p className="text-xs text-muted-foreground mt-1">
                                      {dia?.titulo}
                                    </p>
                                    {Array.isArray(dia?.grupamentos) &&
                                      dia.grupamentos.length > 0 && (
                                        <p className="text-xs text-muted-foreground mt-1">
                                          Grupamentos:{" "}
...
export function mfBuildWeeklyPlanFromState(state: any) {
  const modalities =
    state?.workoutModalities?.length
      ? state.workoutModalities
      : state?.workoutModality
      ? [state.workoutModality]
      : ["musculacao"];
...
function buildDeterministicWeeklyPlan(state: any) {
  ...
  const modalities: string[] =
    Array.isArray(raw.workoutModalities) && raw.workoutModalities.length
  ...
  for (const d of days) {
    const dayPicked = picked.filter((m) =>
      Array.isArray(sched[m]) ? sched[m].includes(d) : false
    );
  ...
  const sessions = days.map((d) => {
    ...
    return {
      day: d,
      modality: m,
      modalityLevel: levels && m ? levels[m] || null : null,
    };
  });
