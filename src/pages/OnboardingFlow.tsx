...
function mfNavGuard(to: string) {
  try {
    ...
    sessionStorage.setItem(
      k,
      JSON.stringify({ t: dt < 1500 ? obj.t : now, n })
    );
    if (n >= 20) {
...
export function OnboardingFlow() {
  ...
  const mfClampStep = (n: number) =>
    Math.max(1, Math.min(TOTAL_STEPS, n));
  ...
  // UNLOCK_FLOW_REDIRECT_EFFECT_V1
  useEffect(() => {
    try {
      const p = loadOnboardingProgress();
      const step =
        p && typeof p.step === "number" && p.step >= 1 && p.step <= TOTAL_STEPS
          ? p.step
          : 1;
  ...
  // 🔚 função de finalização do onboarding (último step)
  const finishOnboarding = () => {
    try {
      const plan = buildActivePlanFromDraft(draft as any);
      saveActivePlan(plan);
    } catch (e) {
      console.error("[OnboardingFlow] erro ao salvar plano ativo:", e);
    }
  ...
  // UNLOCK_STEP_URL_SYNC_V1
  const params = useParams();
  const __clamp = (n: number, a: number, b: number) =>
  ...
  const goNext = () => {
    setActive((x) => {
      // se já estamos no último step, finalizar
      if (x >= TOTAL_STEPS - 1) {
        finishOnboarding();
        return x;
      }
      const nx = x + 1;
      mfGotoStep(nx + 1); // active 0..(TOTAL_STEPS-1) -> step 1..TOTAL_STEPS
      return nx;
    });
  };
    ...
    },
    {
      ...
      render: () => (
        <Step7Preferencias
          ...
          onNext={goNext} // aqui, no último step, goNext chama finishOnboarding
          onBack={goBack}
        />
      ),
    },
    ...
    <div className="w-full max-w-3xl mx-auto px-4 py-6">
      ...
      <div className="mt-3 h-2 w-full rounded-full bg-white/5 border border-white/10 overflow-hidden">
        <div
          ...
          style={{
            width: `${((active + 1) / TOTAL_STEPS) * 100}%`,
          }}
        />
      </div>
      ...
      {/* Navegação mínima / fallback para Steps 1–7 */}
      {
