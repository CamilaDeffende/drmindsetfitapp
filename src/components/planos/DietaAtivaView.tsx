...
function mfSumMeal(refeicao: any) {
  ...
  for (const a of foods) {
    ...
    p += mfNum(a?.proteinas ?? a?.proteina ?? a?.protein ?? a?.proteinG, 0);
    c += mfNum(a?.carboidratos ?? a?.carbs ?? a?.carbsG, 0);
    g += mfNum(a?.gorduras ?? a?.fat ?? a?.fatG, 0);
  }
...
export function DietaAtivaView({ dietaAtiva }: DietaAtivaViewProps) {
  ...
  const refeicoes = Array.isArray((dietaAtiva as any)?.nutricao?.refeicoes)
    ? (dietaAtiva as any).nutricao.refeicoes
    : [];

  const day = useMemo(() => mfSumDay(refeicoes), [refeicoes]);

  const macros = (dietaAtiva as any)?.nutricao?.macros ?? {};

  const alvoKcal = mfNum(
    (dietaAtiva as any)?.nutricao?.caloriasAlvo ??
    (dietaAtiva as any)?.nutricao?.kcalAlvo ??
    macros?.calorias ??
    macros?.kcal ??
    macros?.targetKcal ??
    (dietaAtiva as any)?.caloriasAlvo,
    0
  );
  ...
  const goEdit = () => navigate("/edit-diet");
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape") setEqOpen(null);
    };
    ...
    <>
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <div className="text-xs text-gray-400 mt-1">
                  {(dietaAtiva as any)?.estrategia
                    ? String((dietaAtiva as any).estrategia)
                    : "Plano personalizado"}
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div className="p-3 rounded-xl bg-black/30 border border-white/10">
                ...
                <div className="text-sm text-white mt-1">
                  P <span className="font-semibold">{mfRound(day.p)}g</span> · C <span className="font-semibold">{mfRound(day.c)}g</span> · G <span className="font-semibold">{mfRound(day.g)}g</span>
                </div>
                {(alvoP + alvoC + alvoG) > 0 && (
                  <div className="text-xs text-gray-400 mt-1">
                    Alvo: P <span className="text-white/90">{mfRound(alvoP)}g</span> · C <span className="text-white/90">{mfRound(alvoC)}g</span> · G <span className="text-white/90">{mfRound(alvoG)}g</span>
                  </div>
                )}
              </div>
              ...
              <div className="p-3 rounded-xl bg-black/30 border border-white/10">
                ...
                <div className="text-sm text-white mt-1">
                  {alvoKcal > 0 ? (
                    <span>
                      Diferença: <span className="font-semibold">{mfRound(day.kcal - alvoKcal)} kcal</span>
                    </span>
                  ) : (
                      ...
                      <Card key={idx} className="bg-black/30 border border-white/10">
                        <CardHeader className="py-3">
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <Button variant="outline" className="h-9 px-3 border-white/15 bg-black/40" onClick={goEdit}>
                              Ajustar
                            </Button>
                          </div>
                        </CardHeader>
                          ...
                          {foods.length === 0 ? (
                            ...
                            <div className="space-y-2">
                              {foods.map((a: any, aIdx: number) => {
                                ...
                                const qty = a?.quantidade ?? a?.qty ?? a?.porcao ?? a?.gramas ?? "";
                                const unit = a?.unidade ?? a?.unit ?? (a?.gramas ? "g" : "");
      ...
      {eqOpen && (
        <div
          ...
          onClick={() => setEqOpen(null)}
        >
          <div
            className="w-full max-w-5xl rounded-[24px] border border-white/10 bg-[#071311] shadow-2xl overflow-hidden max-h-[90vh] flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-start justify-between gap-4 px-4 sm:px-6 py-4 border-b border-white/10">
              <div>
                <div className="text-base sm:text-lg font-semibold text-white">
                  Equivalências por kcal
                </div>
                <div className="text-xs sm:text-sm text-gray-400 mt-1">
                  Base: {eqOpen.foodName} · {mfRound(eqOpen.kcal)} kcal
                </div>
              </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="text-xs sm:text-sm text-gray-300 mt-2">
                            Porção equivalente: {mfRound(portion)} {o.unit} · ref {o.kcal100} kcal/100{o.unit}
                          </div>
                          ...
                          <div className="text-[11px] sm:text-xs text-gray-500 mt-3">
