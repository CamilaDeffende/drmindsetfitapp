...
export function WorkoutBuilder() {
  const { workout, addExercise: _addExercise, removeExercise, addSet, updateSet, updateNotes, reset: _reset } = useWorkoutStore();

  const addSession = useHistoryStore((s: any) => s.addSession);

  // marca o início do treino (zera em reset/save)
  ...
  const onSaveWorkout = () => {
    ...
    if (typeof addSession === "function") addSession(session);

    // reinicia a marcação pro próximo treino
    startedAtRef.current = new Date().toISOString();
  };
    ...
    <div style={{ padding: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: tokens.colors.muted }}>Séries • reps • carga • descanso</div>
        </div>

        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={onSaveWorkout} style={{
            borderRadius: 16, border: "1px solid " + tokens.colors.border,
            background: "#0A84FF", color: "#001018",
              ...
              {(ex.sets ?? []).map((s: any, i: number) => (
                <React.Fragment key={i}>
                  ...
                  <input
                    ...
                    type="number"
                    value={s.load}
                    onChange={(e) => updateSet(ex.id, i, { load: Number(e.target.value) })}
                  />
                  <input
                    style={inp}
                    type="number"
                    value={s.restSec ?? 90}
                    onChange={(e) => updateSet(ex.id, i, { restSec: Number(e.target.value) })}
                  />
                </React.Fragment>
              ))}
            </div>
